...
def app_object_detection():
    ...
    # rtsp://admin:admin@192.168.1.101:8554/live           
    def create_player():
        return MediaPlayer("http://admin:admin@101.94.165.138:8081")
    ...
    confidence_threshold = st.slider(
        "置信阈值", 0.0, 1.0, DEFAULT_CONFIDENCE_THRESHOLD, 0.05
    )
    if webrtc_ctx.video_processor:
        webrtc_ctx.video_processor.confidence_threshold = confidence_threshold

    if st.checkbox("显示检测标签", value=True):
